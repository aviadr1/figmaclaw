...
def test_version_prefers_non_editable_local_install_source(tmp_path, monkeypatch) -> None:
    """INVARIANT: local uv tool installs report the installed checkout commit, not stale build info."""

    direct_url = tmp_path / "direct_url.json"
    direct_url.write_text(
        '{"url":"file:///repo/figmaclaw","dir_info":{}}',
        encoding="utf-8",
    )
    ...
    def fake_git_output(_repo, *args):
        ...
        if args == ("log", "-1", "--pretty=%B"):
            return "feat: current local install (#130)"
        return ""
