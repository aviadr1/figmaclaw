...
def test_apply_tokens_emit_only_writes_deterministic_batches(tmp_path: Path) -> None:
    ...
    # Issue #161 / F30: a throw inside the Figma plugin runtime rolls back the
    # entire transaction, so the per-batch summary must NOT throw on
    # hardFailures > 0. Per-row failures are reported via stats counters.
    assert "apply-tokens batch incomplete" not in js
    assert "throw new Error(`apply-tokens" not in js
