...
@click.option("--comp-x", "comp_x", required=True, type=int, help="Canvas x of the component set.")
...
async def _run(
    *,
    ...
    async with FigmaClient(api_key) as client:
        section_data_list = [
            await fetch_section_data(client, source_file_key, section) for section in sections
        ]
