...
> **Status:** authoritative. This document is the single source of truth for figmaclaw's data contract, invariants, and design decisions. Every other doc in `docs/` either feeds into this one (historical context, deeper rationale) or is superseded by it. Module docstrings, commit-message conventions, `CLAUDE.md`, and consumer-repo agent fragments cross-link here for the actual rule.

> When you change figmaclaw's behavior, the order of operations is: (1) update this doc, (2) update tests, (3) write code. If the canon doesn't authorize the behavior, don't ship it.
...
|---|---|---|
| `docs/figmaclaw-canon.md` | This document — invariants, design decisions, refresh ladder, failure modes. | Authoritative. |
